...
class BaseModel(object):
    def __init__(self, args, train, test):
        ...
        if self.args.restart:
            if os.path.exists(checkpoint_path):
                ...
                self.optim_state_dict = self.load_optim_state_dict(
                    checkpoint=checkpoint
                )
                if os.path.exists(self.log_path):
                    print(f"log reindex: {self.log_path}")
                    log_data = (
                        pd.read_csv(self.log_path)
                        .reindex(columns=log_data.split(","))
                        .fillna(0)
                    )
        ...
        self.model_path = (
            self.model_dir
            / f"{self.args.model}_{self.args.dataset}{self.model_name_suffix}{self.eval_suffix}_seed{self.args.seed}.pth"
        )
        self.score_distr_file_path = (
            self.model_dir
            / f"score_distr_{self.args.model}_{self.args.dataset}{self.model_name_suffix}{self.eval_suffix}_seed{self.args.seed}.pickle"
        )
        self.history_img = (
            self.model_dir
            / f"history_{self.args.model}_{self.args.dataset}{self.model_name_suffix}{self.eval_suffix}_seed{self.args.seed}.png"
        )
    ...
    def calc_decision_threshold(self, score_distr_file_path=None):
        ...
        return scipy.stats.gamma.ppf(
            q=self.args.decision_threshold, a=shape_hat, loc=loc_hat, scale=scale_hat
        )
