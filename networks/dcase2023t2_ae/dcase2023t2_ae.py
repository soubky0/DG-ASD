...
class DCASE2023T2AE(BaseModel):
    ...
    def train(self, epoch):
        ...
        for batch_idx, batch in enumerate(tqdm(train_loader)):
            ...
            recon_loss_source = self.loss_reduction(
                score=score[is_source_list], n_loss=n_source
            )
            if n_target > 0:
                recon_loss_target = self.loss_reduction(
                    score=score[is_target_list], n_loss=n_target
                )
            else:
                recon_loss_target = 0
        ...
        if not is_calc_cov:
            ...
            self.fit_anomaly_score_distribution(
                y_pred=y_pred, score_distr_file_path=self.mse_score_distr_file_path
            )
    ...
    def test(self):
        ...
        for idx, test_loader_tmp in enumerate(self.test_loader):
            ...
            with torch.no_grad():
                y_pred, anomaly_score_list, decision_result_list, domain_list = (
                    self.eval(
            ...
            if mode:
                ...
                y_true_s_auc = [
                    ...
                    if domain_list[idx] == "source" or y_true[idx] == 1
                ]
                y_pred_s_auc = [
                    y_pred[idx]
                    for idx in range(len(y_true))
                ...
                y_true_t_auc = [
                    y_true[idx]
                    for idx in range(len(y_true))
                    if domain_list[idx] == "target" or y_true[idx] == 1
                ]
                y_pred_t_auc = [
                    y_pred[idx]
                    for idx in range(len(y_true))
