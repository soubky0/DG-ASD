...
class DCASE202XT2Loader(torch.utils.data.Dataset):
    def __init__(
        ...
        if train:
            ...
            "{target_dir}/{dir_name}".format(
                target_dir=target_dir, dir_name="test_rename"
            )
        ):
        ...
        self.pickle_dir = os.path.abspath(
            "{dir}/processed/{machine_type}/{dir_name}".format(
                dir=root, machine_type=machine_type, dir_name=dir_name
            )
        )
        if not (fmax or fmin):
            fmin_max = ""
        ...
        pickle_path = os.path.abspath(
            f"{self.log_melspectrogram_dir}/{pickle_name}.pickle"
        )
        ...
        if len(self.use_id) > 0:
            idx_list = [
                i
                for i, n in enumerate(np.argmax(self.condition, axis=1))
                if int(section_ids[n]) in self.use_id
            ]
        else:
            idx_list = list(range(len(self.condition)))
        idx_list_2 = [
            i // self.n_vectors_ea_file for i in idx_list[:: self.n_vectors_ea_file]
        ]
    ...
    def load_pre_process(
        ...
        pickle_lockfile_path = os.path.abspath(
            f"{self.log_melspectrogram_dir}/{pickle_name}_lockfile"
        )
        if os.path.isfile(pickle_path) and not os.path.isfile(pickle_lockfile_path):
            ...
            if is_enabled_dataset_dir_lock:
                dataset_dir_lock = fasteners.InterProcessReaderWriterLock(
                    dataset_dir_lockfile_path
                )
            try:
                pickle_lock.acquire_write_lock()
            except:
                print(
                    f"{datetime.datetime.now()}\tcan not lock {pickle_lockfile_path}."
                )
                print(f"{pickle_path} is exists.")
                self.load_pickle(pickle_path=pickle_path)
                return
            ...
            if os.path.exists(pickle_path):
                ...
                com.release_write_lock(
                    lock=pickle_lock, lock_file_path=pickle_lockfile_path
                )
                self.load_pickle(pickle_path=pickle_path)
                return
            ...
            if is_enabled_dataset_dir_lock:
                com.release_read_lock(
                    lock=dataset_dir_lock, lock_file_path=dataset_dir_lockfile_path
                )
    ...
    def default_item(self, index):
        ...
        y_true = (
            self.y_true[index // self.n_vectors_ea_file] if len(self.y_true) > 0 else -1
        )
