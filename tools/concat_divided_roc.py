...
if __name__ == "__main__":
    ...
    parser.add_argument("--parent_dir", type=str, default="", metavar="N", help="")
    parser.add_argument("--export_dir", type=str, default="", metavar="N", help="")
    parser.add_argument(
        "--keyword", type=str, default="_id\((.+)\)", metavar="N", help=""
    )
    parser.add_argument("--machine_type", type=str, default="all", metavar="N", help="")
    parser.add_argument(
        "--file_name", type=str, default="*roc.csv", metavar="N", help=""
    )
    parser.add_argument(
        "--dataset",
        type=str,
    ...
    for machine_type in types:
        ...
        files = sorted(
            glob.glob(
                f"{args.parent_dir}/*{args.dataset}{machine_type}*{args.file_name}"
            )
        )
        ...
        for dev_eval in ["dev", "eval"]:
            if len(file_list[dev_eval]) > 0:
                ...
                all_id_list, nml_id_list = get_use_index_list(
                    file_list[dev_eval], args.keyword
                )
                print(
                    f"{machine_type}\t({dev_eval}_data)\t: {len(nml_id_list)}/{len(all_id_list)}"
                )
                if len(nml_id_list) == len(all_id_list):
                    ...
                    file_path = os.path.splitext(
                        os.path.basename(file_list[dev_eval][0])
                    )[0]
