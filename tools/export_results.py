...
def describe_df(df, df_1d, columns, file_name):
    ...
    describe_1d.loc["hmean"] = stats.hmean(
        np.maximum(df_1d, sys.float_info.epsilon), axis=0
    )
        ...
        for machine_type in machine_type_dict:
            for dev_eval in machine_type_dict[machine_type]:
                new_column_header.append(
                    f"{machine_type}_{dev_eval}_arithmetic mean_{column}"
                )
        new_column_header = [
            new_column
            for new_column in new_column_header
            if new_column in df.columns.values.tolist()
        ]
...
def main(
    ...
    dataset,
    machine_type_dict,
    row_index=["arithmetic mean", "harmonic mean"],
):
    ...
    for target_dir in target_dir_list:
        ...
        for machine_name in machine_type_dict.keys():
            ...
            files = sorted(
                glob.glob(
                    f"{args.parent_dir}/{target_dir}/*{dataset}{machine_name}*roc.csv"
                )
            )
            file_dict = {
            ...
            for dev_eval in file_dict:
                ...
                if len(df_machine_type_1d.index) > 0:
                    ...
                    for column in df_describe.columns.values:
                        summarize_df.at[target_dir, column] = df_describe.at[
                            target_dir, column
                        ]
        ...
        if "pAUC" in auc_header:
            auc_header = [
                column
                for column in auc_header
                if "pAUC" not in column or column == "pAUC"
            ]
        df_mean.loc[target_dir, list(columns_header)] = stats.hmean(
            np.maximum(df_target.loc[:, list(columns_header)], sys.float_info.epsilon),
            axis=0,
        )
        df_mean.loc[target_dir, "TOTAL_hmean"] = stats.hmean(
            np.maximum(df_target.loc[:, auc_header], sys.float_info.epsilon), axis=None
        )
        df_mean.loc[target_dir, "TOTAL_ave"] = (
            df_target.loc[:, auc_header].to_numpy().mean()
        )
        df_mean.loc[target_dir, "TOTAL_std"] = (
            df_target.loc[:, auc_header].to_numpy().std()
        )
        auc_header = [column for column in auc_header if "pAUC" not in column]
        df_mean.loc[target_dir, "AUC_hmean"] = stats.hmean(
            np.maximum(df_target.loc[:, auc_header], sys.float_info.epsilon), axis=None
        )
        df_mean.loc[target_dir, "AUC_ave"] = (
            df_target.loc[:, auc_header].to_numpy().mean()
        )
        df_mean.loc[target_dir, "AUC_std"] = (
            df_target.loc[:, auc_header].to_numpy().std()
        )
