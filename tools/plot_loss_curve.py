...
def csv_to_figdata(
    ...
    ylabel="loss",
    fig_count=1,
    cut_first_epoch=False,
    is_display_console=False,
):
    figdata_list = []
    for i in range(fig_count):
        data_dict = {}
        column_heading_list_temp = (
            column_heading_list[i] if fig_count > 1 else column_heading_list
        )
    ...
    export_path = os.path.join(
        os.path.dirname(file_path), pathlib.PurePath(file_path).stem + ".png"
    )
    show_figs(
        *figdata_list,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--column_heading", type=str, nargs="*", default=["loss"], metavar="N"
    )
