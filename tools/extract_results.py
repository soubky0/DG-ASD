...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-d", "--dev", action="store_true", help="Use Development dataset"
    )
    parser.add_argument(
        "-e", "--eval", action="store_true", help="Use Evaluation dataset"
    )
    ...
    extract_df = pd.DataFrame(
        index=multi_index, columns=SCORE_COLUMNS + machine_type_list
    )
        ...
        for machine_type in machine_type_list:
            ...
            if loc_column in all_summarize_df.columns.values.tolist():
                ...
                for index in all_summarize_df.index:
                    extract_df.at[(index, f"AUC ({domain})"), machine_type] = (
                        all_summarize_df.loc[index, loc_column]
                    )
        if is_pick_mean:
            use_source_target = True
            for index in all_summarize_df.index:
                extract_df.at[(index, f"AUC ({domain})"), "a-mean"] = (
                    all_summarize_df.loc[index, f"ALL_AUC ({domain})_ave"]
                )
                extract_df.at[(index, f"AUC ({domain})"), "h-mean"] = (
                    all_summarize_df.loc[index, f"ALL_AUC ({domain})_hmean"]
                )
    ...
    if not use_source_target:
        for machine_type in machine_type_list:
            ...
            if loc_column in all_summarize_df.columns.values.tolist():
                is_pick_mean = True
                for index in all_summarize_df.index:
                    extract_df.at[(index, "AUC"), machine_type] = all_summarize_df.loc[
                        index, loc_column
                    ]
        if is_pick_mean:
            for index in all_summarize_df.index:
                extract_df.at[(index, "AUC"), "a-mean"] = all_summarize_df.loc[
                    index, f"ALL_AUC_ave"
                ]
                extract_df.at[(index, "AUC"), "h-mean"] = all_summarize_df.loc[
                    index, f"ALL_AUC_hmean"
                ]
    ...
    pickup_columns = ["pAUC"]
    if use_source_target and not any(
        [pickup_columns[0] in c for c in all_summarize_df.columns]
    ):
    ...
    for pickup_column, extract_column in zip(pickup_columns, extract_columns):
        for machine_type in machine_type_list:
            ...
            if loc_column in all_summarize_df.columns.values.tolist():
                is_pick_mean = True
                for index in all_summarize_df.index:
                    extract_df.at[(index, extract_column), machine_type] = (
                        all_summarize_df.loc[index, loc_column]
                    )
        if is_pick_mean:
            for index in all_summarize_df.index:
                extract_df.at[(index, extract_column), "a-mean"] = all_summarize_df.loc[
                    index, f"ALL_{pickup_column}_ave"
                ]
                extract_df.at[(index, extract_column), "h-mean"] = all_summarize_df.loc[
                    index, f"ALL_{pickup_column}_hmean"
                ]

    # Total score
    for index in all_summarize_df.index:
        extract_df.at[(index, "TOTAL score"), "a-mean"] = all_summarize_df.loc[
            index, f"TOTAL_score_ave"
        ]
        extract_df.at[(index, "TOTAL score"), "h-mean"] = all_summarize_df.loc[
            index, f"TOTAL_score_hmean"
        ]

    # Sort results
    extract_df["order"] = [
        SCORE_INDEXES.index(metric) for _, metric in extract_df.index
    ]
    ...
    export_file_path = (
        f"{args.parent_dir}/{args.dataset}_{args.file_name}_extract{args.ext}"
    )
