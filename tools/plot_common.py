...
def show_figs(
    *args,
    ...
    total_height_mm = (
        margin_top_mm
        + margin_bottom_mm
        + (margin_middle_mm + height_mm) * (num_figs_y - 1)
        + height_mm
    )
    total_width_mm = (
        width_mm
        + margin_left_mm
        + margin_right_mm
        + (margin_middle_mm + width_mm) * (num_figs_x - 1)
    )
    ...
    for idx in range(len(args)):
        ...
        x0 = (
            margin_left_inch
            + (width_inch + margin_middle_inch)
            * (num_figs_x - 1 - idx // fold_interval)
        ) / total_width_inch
        y0 = (
            margin_bottom_inch
            + (height_inch + margin_middle_inch)
            * (num_figs_y - 1 - idx % fold_interval)
        ) / total_height_inch
            ...
            if args[idx].type == None or args[idx].type == "plot":
                ...
                for i in range(len(args[idx].highlight_label)):
                    if args[idx].highlight_label[i] >= 0:
                        ...
                        heatmap.add_patch(
                            patches.Rectangle(
                                xy=(args[idx].highlight_label[i], i),
                                width=1,
                                height=1,
                                ec="#00ff00",
                                fill=False,
    ...
    if sup_title:
        fig.suptitle(
            sup_title,
            y=(1 - 0.3 * margin_top_inch / total_height_inch),
            fontsize=title_size,
        )
...
if __name__ == "__main__":
    ...
    y1, sr = librosa.load(
        filename, sr=None, mono=False, offset=3.0, duration=1.0
    )  # read for 3 seconds to 1 second
    y2, sr = librosa.load(
        filename, sr=None, mono=False, offset=6.5, duration=1.0
    )  # read for 6.5 seconds to 1 second
    ...
    f0y_0 = Figdata(
        np.abs(S)[200],
        data2=np.full(len(np.abs(S[200])), 0.1),
        xlabel="freq",
        ylabel="magnitude",
        color="g",
        title="FFT: abs",
    ...
    f1y_l = Figdata(
        y1_l, xlabel="freq", ylabel="magnitude", title="Fig1", ylim=(ymin, ymax)
    )
    f1y_r = Figdata(
        y1_r, xlabel="freq", ylabel="magnitude", title="Fig2", ylim=(ymin, ymax)
    )
    f2y_l = Figdata(
        y2_l,
        data2=np.full(len(y2_l), 0.3),
        color2="yellow",
        xlabel="freq",
        ylabel="magnitude",
        title="Fig3",
    ...
    f2y_r = Figdata(
        y2_r, xlabel="freq", ylabel="magnitude", title="Fig4", ylim=(ymin, ymax)
    )
    s1 = Figdata(
        np.random.randn(100),
